...
def calculate_trc(df):
    ...
    calc_2 = 0.26 * (df.bb - df.ibb + df.hbp)
    calc_3 = 0.52 * (df.sh + df.sf + df.sb)
...
def player_project(
    pa_func, obp_func, slg_func, ops_func, brc_func, src_func, trc_func, data, player
):
    def ab_rate(df):
        return df.ab / df.g
    ...
    player_projection = player_projection.astype(
        {
            "year": "int64",
            "stint": "int64",
            "g": "int64",
