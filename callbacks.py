...
@app.callback(
    [
        Output("team-player-dropdown", "options"),
        Output("team-player-dropdown", "value"),
    ],
...
def update_player_table(player):
    ...
    filter_batter_copy["obp"] = round(
        filter_batter_copy.apply(data.calculate_obp, axis=1), 3
    )
    filter_batter_copy["slg"] = round(
        filter_batter_copy.apply(data.calculate_slg, axis=1), 3
    )
    filter_batter_copy["ops"] = round(
        filter_batter_copy.apply(data.calculate_ops, axis=1), 3
    )
    filter_batter_copy["t_rc"] = round(
        filter_batter_copy.apply(data.calculate_trc, axis=1)
    )
    filter_batter_copy["b_rc"] = round(
        filter_batter_copy.apply(data.calculate_brc, axis=1)
    )
    filter_batter_copy["s_rc"] = round(
        filter_batter_copy.apply(data.calculate_sbrc, axis=1)
    )

    # drop unneccesary columns
    data_filter = filter_batter_copy.drop(
        columns=[
            "player_id",
            "stint",
            "league_id",
...
def update_proj_table(player):
    ...
    ml_data_filter = data.player_project(
        data.calculate_pa,
        data.calculate_obp,
        data.calculate_slg,
        data.calculate_ops,
    ...
    # Return batters data and batters key value pair to columns
    return ml_data_filter.to_dict("records"), [
        {"name": x, "id": x} for x in ml_data_filter
    ]


@app.callback(Output("lrce-bar", "figure"), [Input("team-player-dropdown", "value")])
def update_lrce_player(player):

    filter_year = batter_df[(batter_df["year"] >= 2017) & (batter_df["year"] <= 2020)]
    filter_year.reset_index(drop=True, inplace=True)
    factor_data = data.year_factor(filter_year)
    proj_data = data.player_project(
        data.calculate_pa,
        data.calculate_obp,
        data.calculate_slg,
        data.calculate_ops,
    ...
    fig1 = go.Figure(
        data=[
            go.Bar(
                name="Career Coefficient",
                x=career_lrce.drop(["team_id"], axis=1).columns,
                y=eval_career[0][1::],
                marker_color="#004687",
                opacity=0.8,
            ...
            go.Bar(
                name="Recent Coefficient",
                x=filter_lrce.drop(["team_id"], axis=1).columns,
                y=eval_filter[0][1::],
                marker_color="#AE8F6F",
                opacity=0.8,
                hovertemplate="%{text:.2f}<extra></extra><br>",
